...
void AircraftInstance::addTelemetry(Stamp_t endTime, double flightTimeToRecordMinutes,
    ...
    if (incrementChargingStops)
        ++_telemetry.chargingStops;
}

Stamp_t AircraftInstance::updateTelemetryForCharging(Stamp_t curTime,
...
#ifdef USE_DEBUG_INFO
    std::cout << "aircraft:" << _aircraftId << " updated charge telemetry to {"
              << _telemetry.passengers << "," << _telemetry.totalMilesTraveled << ","
              << _telemetry.totalFaults << "," << _telemetry.chargingStops << ","
              << _telemetry.totalChargingTime << "," << _telemetry.totalFlightHours << "}"
              << std::endl;
#endif
...
Stamp_t AircraftInstance::updateTelemetryForFlight(Stamp_t curTime, Stamp_t endTime) {
    curTime = std::min(curTime, endTime); // clip to end of simulation time.
    ...
    Stamp_t nextChargeTime = static_cast<Stamp_t>(curTime + this->flightTimePerCharge());
#ifdef USE_DEBUG_INFO
    std::cout << "aircraft:" << _aircraftId << " updated flight telemetry to {"
              << _telemetry.passengers << "," << _telemetry.totalMilesTraveled << ","
              << _telemetry.totalFaults << "," << _telemetry.chargingStops << ","
              << _telemetry.totalChargingTime << "," << _telemetry.totalFlightHours << "}"
              << std::endl;
