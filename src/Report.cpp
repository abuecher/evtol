...
void Report::addAircraftTelemetry(int aircraftId, const std::string& modelName,
                                  const Telemetry& telemetry) {
    std::set<int> aircraftReported;
    if (aircraftReported.find(aircraftId) != aircraftReported.end()) {
        std::cout << "dedupe - aircraft " << aircraftId << " telemetry already reported"
                  << std::endl;
        return;
    }
...
void Report::writeReport(std::ostream& out, char delim) const {
    // table header
    out << "Model" << delim << "aircraft-count" << delim << "avg-flight-time" << delim
        << "avg-distance-per-flight" << delim << "avg-time-charging-per-charge-session" << delim
        << "total-faults" << delim << "total-passenger-miles\n";
    for (const auto item : this->totalModelTelemetry) {
        ...
        out << item.first << delim << instanceCount << delim << avg_flight_time_min / 60 << delim
            << avg_distance_per_flight << delim << avg_charging_per_session / 60 << delim << faults
            << delim << totalPassMiles << "\n";
    }
}

std::map<std::string, Telemetry> Report::getCopyOfTelemetry() const {
    return std::map<std::string, Telemetry>(this->totalModelTelemetry.begin(),
                                            this->totalModelTelemetry.end());
}
